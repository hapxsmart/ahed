 ...
 */
class MassSuspend extends Action
{
    /**
     * {@inheritdoc}
     */
        ...
        try {
            ...
            foreach ($collection->getAllIds() as $profileId) {
                ...
                if (in_array(Status::SUSPENDED, $allowedStatuses)) {
                    try {
                        $this->profileManagement->changeStatusAction($profileId, Status::SUSPENDED);
                        $successCount++;
            ...
            $this->messageManager->addSuccessMessage(
                __('A total of %1 record(s) have been suspended.', $successCount)
            );
            ...
            $this->messageManager->addExceptionMessage(
                $exception,
                __('Something went wrong while suspend the items.')
            );
        }
